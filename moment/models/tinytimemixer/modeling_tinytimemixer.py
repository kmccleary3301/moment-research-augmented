...
class TinyTimeMixerAttention(nn.Module):
    ...
    def __init__(
        ...
        if self.use_sparse_attention:
            try:
                ...
                # Define appropriate sliding window and block sizes based on typical use
                sliding_window_size = config.patch_length if hasattr(config, 'patch_length') else 16
                compress_block_size = sliding_window_size
                selection_block_size = sliding_window_size
                num_selected_blocks = 2  # Default value, can be tuned
                ...
                print("Warning: native_sparse_attention_pytorch not found. Falling back to regular attention.")
    ...
    def forward(
        ...
        if self.use_sparse_attention:
            # SparseAttention expects input of shape [batch_size, seq_len, dim]
            # Return shape: [batch_size, seq_len, dim]
            attn_output = self.sparse_attention(hidden_states)
            
            # For compatibility with return signature of regular attention
            # Return tuple of (output, attention weights, None)
            return attn_output, None, None
        
        # Regular attention implementation (existing code)
