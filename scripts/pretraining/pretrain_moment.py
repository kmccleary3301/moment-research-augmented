#!/usr/bin/env python
# Pretraining script for MOMENT model on the TimeSeriesPILE dataset
...
try:
    ...
    sys.path.append(os.path.join(os.path.dirname(__file__), 'cloned_model_repos/moment_augmented'))
    from momentfm.models.moment import MOMENT, MOMENTPipeline
    from momentfm.utils.utils import NamespaceWithDefaults

# Import the modified dataset loader
# from momentfm.data.modified_dataset_loader import TimeSeriesPILEDataset, set_seed
...
def create_model(args, config):
    """Create and initialize the MOMENT model."""
    try:
        # Create a Namespace from the config dictionary
        from argparse import Namespace
        config_namespace = Namespace(**config)
        
        print("moment model config:", config)
        # Convert to NamespaceWithDefaults
        model_config = NamespaceWithDefaults.from_namespace(config_namespace)
        
        # Initialize the model
        model = MOMENTPipeline(model_config)
        
        print(json.dumps(model.config.to_json(), indent=4))
        
        logger.info(f"Created MOMENT model with {sum(p.numel() for p in model.parameters()):,} parameters")
        return model
...
def create_scheduler(optimizer, args, steps_per_epoch):
    ...
    # Ensure we have at least 1 step for the scheduler
    steps_per_epoch = max(1, steps_per_epoch)
    total_steps = steps_per_epoch * args.epochs
    
    # Calculate warmup percentage, capped between 0 and 0.5
    pct_start = min(0.5, max(0.1, args.warmup_steps / max(1, total_steps)))
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Pretrain MOMENT on the TimeSeriesPILE dataset")
    ...
    parser.add_argument('--test_sparse_attention', action='store_true', help='Only test sparse attention initialization')
    ...
    parser.add_argument('--output_dir', type=str, default='output/moment', help='Output directory for checkpoints and logs')
    ...
    if args.model_label is None:
        args.model_label = "MOMENT (NSA)" if args.use_sparse_attention else "MOMENT"
...
def main():
    ...
    # Test sparse attention initialization if requested
    if args.test_sparse_attention:
        # Setup logging
        setup_logging(args)
        logger.info("Testing sparse attention initialization...")
        ...
        try:
            ...
            # Create a regular Namespace from the dictionary
            from argparse import Namespace
            config_namespace = Namespace(**config)
            
            # Convert the Namespace to NamespaceWithDefaults
            from momentfm.utils.utils import NamespaceWithDefaults
            model_config = NamespaceWithDefaults.from_namespace(config_namespace)
            ...
            logger.info("Successfully initialized model with sparse attention!")
            logger.info("You can now run the full training script.")
            return
        except Exception as e:
            logger.error(f"Failed to initialize model with sparse attention: {e}")
            import traceback
    ...
    hf_model_dir = os.path.join(args.output_dir, "hf_model")
    ...
    # Final checkpoint
    final_checkpoint_path = os.path.join(hf_model_dir, "checkpoint_final.pt")
    save_checkpoint(
        model, optimizer, scheduler, epoch, train_loss, val_loss, 
    ...
    if len(continuous_train_losses) > 0:
        ...
        all_detailed_loss_data = {
            ...
            "train_tokens": continuous_train_tokens, # Is a list
            "val_tokens": continuous_val_tokens,
            "smoothed_train_losses": smoothed_train_losses, # Is a list
            "smoothed_val_losses": smoothed_val_losses, # Is a list
            "smoothed_train_tokens": smoothed_train_tokens, # Is a list
            "smoothed_val_tokens": smoothed_val_tokens, # Is a list
        }
