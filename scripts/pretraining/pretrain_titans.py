#!/usr/bin/env python
# Pretraining script for TITANsTS model on the TimeSeriesPILE dataset
...
def create_model(args, config):
    """Create and initialize the TITANsTS model with MOMENT compatibility layer."""
    try:
        # Create a Namespace from the config dictionary
        config_namespace = Namespace(**config)
        
        logger.info("TITANsTS model config: %s", json.dumps(config, indent=2))
        ...
        if hasattr(args, 'debug'):
            ...
            model_config.adapter_debug = args.debug_verbose
            if args.debug:
                logger.info("Debug mode enabled for model")
        
        # Initialize the TITANsTS adapter model
        model = TITANsTSAdapter(model_config, logger=logger)
        
        # Log model parameters
        param_count = sum(p.numel() for p in model.parameters())
        trainable_param_count = sum(p.numel() for p in model.parameters() if p.requires_grad)
        logger.info(f"Created TITANsTS model with {param_count:,} parameters ({trainable_param_count:,} trainable)")
        ...
        try:
            ...
            logger.warning("Failed to enable gradient checkpointing for TITANs model: TITANsTSAdapter does not support gradient checkpointing.")
        
        # Log key model configuration
        logger.info(f"Model dimension: {model_config.d_model}, Layers: {model_config.num_layers}")
        logger.info(f"Window size: {model_config.window_size}")
...
def save_checkpoint(model, optimizer, scheduler, epoch, train_loss, val_loss, 
    ...
    if token_counter is not None:
        checkpoint['token_counter'] = token_counter.get_tokens()
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description="Pretrain TITANsTS on the TimeSeriesPILE dataset")
    ...
    parser.add_argument('--num_layers', type=int, default=None, 
                        help='Number of transformer layers (overrides model_size setting if provided)')
    parser.add_argument('--window_size', type=int, default=64, 
                        help='Sliding window size for segmented attention')
    parser.add_argument('--memory_heads', type=int, default=None, 
                        help='Number of memory attention heads (overrides model_size setting if provided)')
    parser.add_argument('--num_persist_mem', type=int, default=None, 
                        help='Number of persistent memory tokens (overrides model_size setting if provided)')
    parser.add_argument('--num_longterm_mem', type=int, default=None, 
                        help='Number of long-term memory tokens (overrides model_size setting if provided)')
    parser.add_argument('--neural_memory_depth', type=int, default=2,
                        help='Depth of the neural memory network')
    parser.add_argument('--neural_mem_layers', nargs='+', type=int, default=None,
                        help='List of layer indices that have neural memory')
    parser.add_argument('--memory_dim', type=int, default=None,
                        help='Dimension of the memory model')
    parser.add_argument('--neural_mem_segment_len', type=int, default=4,
                        help='Segment length for neural memory processing')
    parser.add_argument('--neural_mem_batch_size', type=int, default=128,
                        help='Batch size for neural memory processing')
    parser.add_argument('--use_neural_memory_attention', action='store_true',
                        help='Use MemoryAttention instead of MemoryMLP')
    ...
    parser.add_argument('--output_dir', type=str, default='output/titans', help='Output directory for checkpoints and logs')
...
def get_model_config(args):
    ...
    # Override with commandline arguments if provided
    ...
    if args.memory_heads is not None:
        config['memory_heads'] = args.memory_heads
        
    if args.num_persist_mem is not None:
        config['num_persist_mem'] = args.num_persist_mem
        
    if args.num_longterm_mem is not None:
        config['num_longterm_mem'] = args.num_longterm_mem
        
    if args.neural_mem_layers is not None:
        config['neural_mem_layers'] = tuple(args.neural_mem_layers)
        
    if args.memory_dim is not None:
        config['memory_dim'] = args.memory_dim
        
    if args.neural_mem_segment_len is not None:
        config['neural_mem_segment_len'] = args.neural_mem_segment_len
        
    if args.neural_mem_batch_size is not None:
        config['neural_mem_batch_size'] = args.neural_mem_batch_size
    
    # Add configuration for neural memory attention vs MLP
    config['use_neural_memory_attention'] = args.use_neural_memory_attention
    config['neural_memory_depth'] = args.neural_memory_depth
    
    # Set sparse attention parameters
    config['use_sparse_attention'] = args.use_sparse_attention
    if args.use_sparse_attention:
        config['sparse_attention_params'] = {
